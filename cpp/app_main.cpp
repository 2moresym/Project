...
QString detectProvider() {
    ...
    if (!qEnvironmentVariable("OPENAI_API_KEY").trimmed().isEmpty()) return QStringLiteral("openai");
...
QFont emojiFont() {
    ...
    for (const QString& family : {
        ...
        if (available.contains(family)) families << family;
    }
...
public:
    explicit SettingsDialog(QWidget* parent = nullptr) : QDialog(parent) {
        ...
        auto* note = new QLabel(QStringLiteral("Credentials stay in local desktop settings."), this);
...
private:
    static void fillModels(QComboBox* box, const QStringList& models, const QString& selected) {
        box->clear();
        box->addItems(models);
        if (!selected.isEmpty() && !models.contains(selected)) box->addItem(selected);
        if (!selected.isEmpty()) box->setCurrentText(selected);
    }

    void load() {
        QSettings s;
        const QString provider = detectProvider();
        m_provider->setCurrentIndex(provider == QStringLiteral("openai") ? 1 : 0);
        m_providerPages->setCurrentIndex(provider == QStringLiteral("openai") ? 1 : 0);
...
public:
    NativeWindow() : m_backend(new QProcess(this)) {
        ...
        buildUi();
        startBackend();
        if (!hasCredential(detectProvider())) QTimer::singleShot(250, this, &NativeWindow::showSettings);
    }
...
private:
    void buildUi() {
        ...
        connect(m_backend, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            ...
            if (status == QProcess::CrashExit) setStatus(QStringLiteral("Backend crashed"));
        });
    ...
    void applyStyle() {
        setStyleSheet(QStringLiteral(
            "QMainWindow,QWidget{background:#0e1117;color:#eef2f7;font-family:'Noto Sans','Noto Color Emoji','Sans Serif';font-size:11pt;}"
            ...
            "QMenu::item:selected{background:#2b3b5e;color:#fff;}"
            "QToolTip{background:#171d27;color:#eef2f7;border:1px solid #2a3342;}"
        ));
        qApp->setFont(emojiFont());
    }
    ...
    void startBackend() {
        ...
        m_backend->start();
        log(QStringLiteral("backend start program=%1 provider=%2").arg(m_backend->program(), detectProvider()));
    }
    ...
    void sendMessage() {
        ...
        if (!m_backend->waitForStarted(1200)) {
            setStatus(QStringLiteral("Backend unavailable"));
            return;
        }
    ...
    void showDebug() {
        m_logs << QStringLiteral("[%1] backend state=%2 program=%3 provider=%4")
            .arg(QDateTime::currentDateTime().toString(Qt::ISODate), QString::number(m_backend->state()), m_backend->program(), detectProvider());
