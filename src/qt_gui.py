...
class MainWindow(QMainWindow):
    ...
    def _apply_theme(self) -> None:
        ...
        popup = "#20242d" if dark else "#ffffff"
        accents = {"default": "#5b8cff", "cyan": "#22d3ee", "green": "#22c55e", "magenta": "#e879f9"}
        accent = accents.get(self.settings.theme, accents["default"])
        self.setStyleSheet(f"""
            QMainWindow, QWidget {{ background: {bg}; color: {text}; font-family: 'Noto Sans'; font-size: 11pt; }}
            QLabel, QCheckBox {{ background: transparent; color: {text}; }}
            #sidebar {{ background: {panel}; border: 1px solid {border}; border-radius: 18px; }}
            #appTitle {{ background: transparent; font-size: 18pt; font-weight: 700; }}
            #header {{ background: transparent; font-size: 14pt; font-weight: 650; padding: 6px; }}
            #muted {{ background: transparent; color: {muted}; font-size: 9pt; }}
            QPushButton, QToolButton {{ background: {panel}; color: {text}; border: 1px solid {border}; border-radius: 11px; padding: 9px 13px; }}
            ...
            QPushButton#primary {{ background: {accent}; color: white; border: 0; font-weight: 650; }}
            QPushButton#primary:hover {{ background: {accent}; }}
            QListWidget, QTextBrowser, QTextEdit, QLineEdit, QComboBox {{ background: {input_bg}; color: {text}; border: 1px solid {border}; border-radius: 15px; padding: 10px; selection-background-color: {accent}; selection-color: white; }}
            QListWidget::item {{ padding: 8px; border-radius: 8px; }}
            QListWidget::item:selected {{ background: {accent}; color: white; }}
            QComboBox {{ padding-right: 30px; }}
            QComboBox QAbstractItemView {{ background: {popup}; color: {text}; border: 1px solid {border}; selection-background-color: {accent}; selection-color: white; }}
            QComboBox::drop-down {{ width: 28px; border: 0; background: transparent; }}
            QComboBox::down-arrow {{ width: 0; height: 0; border: 0; }}
    ...
    def _show_chat(self) -> None:
        ...
        for message in self.chat.messages:
            ...
            blocks.append(f"<div style='margin: 12px 4px'><div><b>{html.escape(who)}</b></div><div style='margin-top: 6px'>{self._html_response(message['content'])}</div></div>")
    ...
    def _finish_reply(self, answer: str, error: object) -> None:
        if error:
            ...
            if self.settings.auto_memory and len(self.chat.messages) >= 2:
    ...
    def _set_sidebar_width(self, width: int) -> None:
        ...
        self.sidebar.setMaximumWidth(max(0, width))
        if width > 0 and not self.sidebar.isVisible():
    ...
    def toggle_sidebar(self) -> None:
        ...
        width_anim.setStartValue(start if not opening else 0)
