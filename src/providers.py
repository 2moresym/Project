...
class DemoProvider:
    ...
    def stream_reply(self, messages): yield self.reply(messages)
...
class OpenAICompatibleProvider:
    ...
    def _request(self, messages, stream=False):
        ...
        except TimeoutError as exc: raise RuntimeError("API request timed out.") from exc
    ...
    def stream_reply(self,messages):
        with self._request(messages,True) as response:
            for raw in response:
                ...
                try: piece=data["choices"][0].get("delta",{}).get("content","")
