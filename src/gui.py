...
class VaxxApp(tk.Tk):
    ...
    def _retheme_widgets(self):
        self._apply_theme(); self._build(); self._refresh_chats(); self._show_chat()
    ...
    def _build(self):
        ...
        self.side.grid_rowconfigure(3,weight=1); self.side.grid_columnconfigure(0,weight=1)
        head = ttk.Frame(self.side,style="Panel.TFrame"); head.grid(row=0,column=0,sticky="ew",pady=(0,2)); head.grid_columnconfigure(0,weight=1)
        ...
        self.chat_list = tk.Listbox(self.side,exportselection=False,activestyle="none",font=self._font(),bg=self.palette["panel"],fg=self.palette["text"],selectbackground=self.palette["accent"],selectforeground="#ffffff",highlightthickness=1,highlightbackground=self.palette["border"],relief="flat",bd=0)
        self.chat_list.grid(row=3,column=0,sticky="nsew",pady=(2,8)); self.chat_list.bind("<<ListboxSelect>>",self._select_chat)
        for row,text,cmd in [(4,"Rename chat",self._rename_chat),(5,"Delete chat",self._delete_chat),(6,"Memory",self._show_memory),(7,"Settings",self._settings)]:
        ...
        top = ttk.Frame(self.main); top.grid(row=0,column=0,sticky="ew",pady=(0,8)); top.grid_columnconfigure(0,weight=1)
        ttk.Label(top,text=f"{self._chat().ai_name}  ·  {self.current}",font=self._font(14,True),style="TLabel").grid(row=0,column=0,sticky="w")
        ...
        body.grid(row=1,column=0,sticky="nsew"); body.grid_rowconfigure(0,weight=1); body.grid_columnconfigure(0,weight=1)
        self.output = tk.Text(body,wrap="word",state="normal",padx=18,pady=16,font=self._font(),bg=self.palette["panel"],fg=self.palette["text"],insertbackground=self.palette["text"],selectbackground=self.palette["accent"],relief="flat",bd=0,spacing3=5)
        self.output.grid(row=0,column=0,sticky="nsew")
        self.output.bind("<Key>",self._block_chat_edit); self.output.bind("<Control-c>",self._copy_selection); self.output.bind("<Control-a>",self._select_all_output); self.output.bind("<Button-3>",self._chat_context_menu)
        ...
        bottom = tk.Frame(self.main,bg=self.palette["bg"]); bottom.grid(row=2,column=0,sticky="ew",pady=(10,0)); bottom.grid_columnconfigure(0,weight=1)
        input_frame = tk.Frame(bottom,bg=self.palette["input"],highlightbackground=self.palette["border"],highlightthickness=1,bd=0); input_frame.grid(row=0,column=0,sticky="ew"); input_frame.grid_columnconfigure(0,weight=1)
        self.entry = tk.Text(input_frame,height=3,wrap="word",font=self._font(),bg=self.palette["input"],fg=self.palette["text"],insertbackground=self.palette["text"],relief="flat",bd=0,padx=10,pady=8)
        self.entry.grid(row=0,column=0,sticky="ew"); self.entry.bind("<Control-Return>",lambda _:self._send())
    ...
    def _chat_context_menu(self,event):
        ...
        menu.add_command(label="Copy",command=self._copy_selection); menu.add_command(label="Select all",command=self._select_all_output); menu.tk_popup(event.x_root,event.y_root)
    ...
    def _update_sidebar_visibility(self):
        if self.sidebar_open:
            self.side.grid(); self.grid_columnconfigure(0,minsize=255)
        else:
            self.side.grid_remove(); self.grid_columnconfigure(0,minsize=0)
    ...
    def _show_chat(self):
        ...
        self.title(f"{APP_NAME} — {chat.ai_name}"); self.output.delete("1.0","end")
        if not chat.messages:self.output.insert("end","Start a conversation with Vaxx.\n")
        for message in chat.messages:
            if message.get("role")=="system":continue
            who="You" if message["role"]=="user" else chat.ai_name; self.output.insert("end",f"{who}\n",("role_you" if who=="You" else "role_ai")); self.output.insert("end",render(message["content"])+"\n\n")
        self.output.tag_config("role_you",foreground=self.palette["accent"],font=self._font(10,True)); self.output.tag_config("role_ai",foreground=self.palette["text"],font=self._font(10,True)); self.output.see("end")

    def _append(self,who,text): self.output.insert("end",f"{who}\n",("role_you" if who=="You" else "role_ai")); self.output.insert("end",render(text)+"\n\n"); self.output.see("end")
