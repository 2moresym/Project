...
class VaxxApp(tk.Tk):
    def __init__(self):
        ...
        self.title(f"{self.store.chats[self.current].ai_name} — Tiny AI Playground"); self.geometry("1050x700"); self.minsize(760,500); self._build(); self._refresh_chats(); self._show_chat(); self.protocol("WM_DELETE_WINDOW",self._quit)
    def _build(self):
        ...
        body=ttk.Frame(main); body.grid(row=1,column=0,sticky="nsew"); body.grid_rowconfigure(0,weight=1); body.grid_columnconfigure(0,weight=1); self.output=tk.Text(body,wrap="word",state="disabled",padx=16,pady=14,font=("TkDefaultFont",11)); self.output.grid(row=0,column=0,sticky="nsew"); scroll=ttk.Scrollbar(body,command=self.output.yview); scroll.grid(row=0,column=1,sticky="ns"); self.output.configure(yscrollcommand=scroll.set)
    ...
    def _show_chat(self):
        chat=self._chat(); self.title(f"{chat.ai_name} — Tiny AI Playground"); self.header.config(text=f"{chat.ai_name}  ·  {self.current}  ·  {self.settings.model}"); self.output.config(state="normal"); self.output.delete("1.0","end")
    ...
    def _rename_chat(self):
        ...
        old=self.current; chat=self.store.chats[old]; self.store.rename(old,key); self.store.chats[key]=chat; self.current=key; self.settings.current_chat=key; self.settings.save(); self._refresh_chats(); self._show_chat()
