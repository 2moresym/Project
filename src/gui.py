...
class VaxxApp(tk.Tk):
    ...
    def _apply_theme(self):
        self.palette=PALETTES[effective_appearance(self.settings.appearance)]; self.configure(bg=self.palette["bg"]); style=ttk.Style(self); style.theme_use("clam")
    ...
    def _retheme_widgets(self):
        self._apply_theme(); self._build(True); self._refresh_chats(); self._show_chat()
    def _build(self, rebuild=False):
        ...
        ttk.Label(side,text="Lightweight • private local state",font=self._font(9),style="Panel.TLabel").grid(row=0,column=0,sticky="w",pady=(34,0))
        ...
        self.chat_list.grid(row=2,column=0,sticky="nsew",pady=(2,8)); self.chat_list.bind("<<ListboxSelect>>",self._select_chat)
        ...
        self.output.grid(row=0,column=0,sticky="nsew"); scroll=ttk.Scrollbar(body,command=self.output.yview); scroll.grid(row=0,column=1,sticky="ns"); self.output.configure(yscrollcommand=scroll.set)
        bottom=tk.Frame(main,bg=self.palette["bg"]); bottom.grid(row=2,column=0,sticky="ew",pady=(10,0)); bottom.grid_columnconfigure(0,weight=1)
        ...
        self.entry=tk.Text(input_frame,height=3,wrap="word",font=self._font(),bg=self.palette["input"],fg=self.palette["text"],insertbackground=self.palette["text"],relief="flat",bd=0,padx=10,pady=8); self.entry.grid(row=0,column=0,sticky="ew"); self.entry.bind("<Control-Return>",lambda _:self._send())
    ...
    def _settings(self):
        win=tk.Toplevel(self); win.title("Settings"); win.transient(self); win.grab_set(); win.resizable(False,False); win.configure(bg=self.palette["bg"])
        frame=tk.Frame(win,bg=self.palette["panel"],highlightbackground=self.palette["border"],highlightthickness=1,padx=22,pady=18); frame.grid(row=0,column=0,padx=14,pady=14)
        label=lambda text,row: tk.Label(frame,text=text,bg=self.palette["panel"],fg=self.palette["text"],font=self._font(10,True)).grid(row=row,column=0,sticky="w",padx=(0,20),pady=7)
        ttk.Style(win).configure("Settings.TCombobox",fieldbackground=self.palette["input"],background=self.palette["input"],foreground=self.palette["text"])
