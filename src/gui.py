...
class RoundedButton(tk.Canvas):
    def __init__(self, master, text, command, palette, accent=False, **kwargs):
        ...
        self.bind("<Configure>", lambda _: self._draw(False))
        self._draw(False)
    ...
    def _draw(self, hover):
        ...
        fill = self.palette["accent"] if self.accent else self.palette["input"]
        fg = self.palette["accent_fg"] if self.accent else self.palette["text"]
        border = self.palette["accent"] if (hover or self.accent) else self.palette["border"]
        self.create_polygon(_rounded_points(1, 1, w-1, h-1, 11), smooth=True, splinesteps=16, fill=fill, outline=border, width=1)
        family = "Noto Sans" if "Noto Sans" in self.winfo_toplevel().tk.call("font", "families") else "TkDefaultFont"
        self.create_text(w/2, h/2, text=self.text, fill=fg, font=(family, 10, "bold" if self.accent else "normal"))
...
class VaxxApp(tk.Tk):
    ...
    def _apply_theme(self):
        ...
        accent, soft = ACCENTS.get(self.settings.theme, ACCENTS["default"])
        self.palette["accent"] = accent
        self.palette["user_bg"] = soft
        self.palette["accent_fg"] = "#ffffff"
    ...
    def _build(self, rebuild=False):
        ...
        side = ttk.Frame(self, style="Panel.TFrame", padding=14)
        side.grid(row=0, column=0, sticky="nsew", padx=(12, 7), pady=12)
        side.grid_rowconfigure(3, weight=1)

        ttk.Label(side, text=APP_NAME, font=self._font(19, True), style="Panel.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(side, text="Lightweight • private local state", font=self._font(9), style="Panel.TLabel").grid(row=1, column=0, sticky="w", pady=(2, 12))
        RoundedButton(side, "＋ New chat", self._new_chat, self.palette, accent=True).grid(row=2, column=0, sticky="ew", pady=(0, 8))

        self.chat_list = tk.Listbox(side, exportselection=False, activestyle="none", font=self._font(), bg=self.palette["panel"], fg=self.palette["text"], selectbackground=self.palette["accent"], selectforeground=self.palette["accent_fg"], highlightthickness=1, highlightbackground=self.palette["border"], relief="flat", bd=0)
        ...
        for row, text, cmd in [(4, "Rename chat", self._rename_chat), (5, "Delete chat", self._delete_chat), (6, "Memory", self._show_memory), (7, "Settings", self._settings)]:
            RoundedButton(side, text, cmd, self.palette).grid(row=row, column=0, sticky="ew", pady=3)

        main = ttk.Frame(self, padding=(8, 12, 12, 12))
        ...
        body = tk.Frame(main, bg=self.palette["panel"], highlightbackground=self.palette["border"], highlightthickness=1, bd=0)
        body.grid(row=1, column=0, sticky="nsew")
        body.grid_rowconfigure(0, weight=1)
        body.grid_columnconfigure(0, weight=1)
        self.output = tk.Text(body, wrap="word", state="disabled", padx=18, pady=16, font=self._font(), bg=self.palette["panel"], fg=self.palette["text"], insertbackground=self.palette["text"], selectbackground=self.palette["accent"], relief="flat", bd=0, spacing3=5)
        self.output.grid(row=0, column=0, sticky="nsew")
        scroll = ttk.Scrollbar(body, command=self.output.yview)
        scroll.grid(row=0, column=1, sticky="ns")
        self.output.configure(yscrollcommand=scroll.set)

        bottom = tk.Frame(main, bg=self.palette["bg"])
        ...
        RoundedButton(bottom, "Send", self._send, self.palette, accent=True).grid(row=0, column=1, sticky="ns", padx=(8, 0))
        ttk.Label(main, text="Ctrl+Enter to send • chats and settings save locally", foreground=self.palette["muted"], style="TLabel", font=self._font(9)).grid(row=3, column=0, sticky="w", pady=(6, 0))
    ...
    def _select_chat(self, _=None):
        ...
        if sel:
            self.current = list(self.store.chats)[sel[0]]; self.settings.current_chat = self.current; self.settings.save(); self._show_chat()
    ...
    def _show_chat(self):
        ...
        for message in chat.messages:
            ...
            self.output.insert("end", render(message["content"]) + "\n\n")
    ...
    def _settings(self):
        ...
        model_var = tk.StringVar(value=self.settings.model); label("Model", 3); ttk.Combobox(frame, textvariable=model_var, state="readonly", values=model_values, width=41, style="Settings.TCombobox").grid(row=3, column=1, sticky="ew", pady=7)
        ...
        for row, text, var in [(6, "Enable streaming responses", stream_var), (7, "Automatically remember simple personal facts", memory_var), (8, "Automatically summarize long conversations", summary_var)]:
        ...
        buttons = tk.Frame(frame, bg=self.palette["panel"]); buttons.grid(row=10, column=0, columnspan=2, sticky="e", pady=(14, 0))
        RoundedButton(buttons, "Cancel", win.destroy, self.palette).grid(row=0, column=0, padx=(0, 8))
        def save_settings():
        ...
        RoundedButton(buttons, "Save", save_settings, self.palette, accent=True).grid(row=0, column=1)
        ...
        win.geometry(f"+{x}+{y}")
        # Defer modal grab until the Toplevel is mapped; calling grab_set too early
        # can fail on some Tk/X11 builds with "window not viewable".
        win.after_idle(win.grab_set); win.after_idle(win.focus_force)

    def _quit(self):
        try:
            self.store.save(self.current, self._chat()); self.settings.current_chat = self.current; self.settings.save()
