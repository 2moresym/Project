...
def model_menu(chat,s,fd):
    old=termios.tcgetattr(fd); opts=MODELS+["Cancel"]; i=menu("SELECT MODEL",opts,s,fd,MODELS.index(s.model) if s.model in MODELS else 0)
    if i is not None and i<len(MODELS):s.model=MODELS[i];rebuild(chat,s);s.save()
def provider_menu(chat,s,fd):
    old=termios.tcgetattr(fd);opts=["Hugging Face (HF_TOKEN)","OpenAI-compatible (OPENAI_API_KEY)","Cancel"];i=menu("SELECT API PROVIDER",opts,s,fd,0 if s.provider=="huggingface" else 1)
