...
class Chat:
    ...
    def context_messages(self):
        ...
        if self.memories: parts.append("Persistent memory:\n"+"\n".join(f"- {m}" for m in self.memories))
        if self.summary: parts.append(f"Conversation summary:\n{self.summary}")
    ...
    def send(self,text):
        ...
        except Exception: self.messages.pop(); raise
        self.messages.append({"role":"assistant","content":answer}); return answer
    ...
    def auto_remember(self,text):
        ...
        if not text.lower().startswith(("my name is ","i'm ","i am ","i like ","i love ","i prefer ","i use ","i'm working on ","i am working on ")): return []
        fact=text.strip().rstrip(".!?")
        return [fact] if self.remember(fact) else []

    def maybe_summarize(self, limit=32, keep=12):
        if len(self.messages)<=limit: return False
        old=self.messages[:-keep]
        prompt=[{"role":"system","content":"Summarize this conversation for another assistant. Preserve names, decisions, goals, important facts and unresolved questions. Be concise."},{"role":"user","content":"\n".join(f"{m['role']}: {m['content']}" for m in old)}]
        try: summary=self.provider.reply(prompt)
    ...
    def search(self,query):
        q=query.lower().strip(); return [m for m in self.messages if q in m.get("content","").lower()] if q else []

    def to_data(self): return {"version":4,"ai_name":self.ai_name,"memories":self.memories,"summary":self.summary,"messages":self.messages}
    ...
    def save(self):
        DATA_DIR.mkdir(parents=True,exist_ok=True); self._write(HISTORY_FILE)

    def _write(self,path):
        temp=path.with_suffix(path.suffix+".tmp"); temp.write_text(json.dumps(self.to_data(),indent=2,ensure_ascii=False),encoding="utf-8"); temp.replace(path)

    def load(self):
        if not HISTORY_FILE.exists(): return
        try:
            data=json.loads(HISTORY_FILE.read_text(encoding="utf-8"))
            if isinstance(data,list): self.messages=[m for m in data if isinstance(m,dict)]; return
            self.from_data(data)
        except (OSError,json.JSONDecodeError) as exc: print(f"Warning: couldn't load legacy history: {exc}",file=sys.stderr)
